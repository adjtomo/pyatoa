...
class WaveMaker:
    ...
    def plot_stalta(self, ax, stalta, plot_waterlevel=True):
        ...
        if plot_waterlevel:
            ...
            ax.annotate(text=f"stalta_waterlevel = {stalta_wl}", alpha=0.7, 
                        fontsize=fontsize,
        ...
        for j, window in enumerate(windows):
            ...
            if plot_window_annos:
                ...
                s_anno = window_anno.format(
                                ...
                                left=tleft,
                                length=tright - tleft)
                ax.annotate(text=s_anno, ha=window_anno_ha, va=window_anno_va,
                            xy=(t_anno, y_anno),
            ...
            if plot_phase_arrivals:
                for phase_arrivals in window.phase_arrivals:
                    if phase_arrivals["name"] in ["p", "s"]:
                        ...
                        ax.annotate(text=phase_arrivals["name"],
        ...
        for tag in rejwin.keys():
            ...
            if rwin_arr.any():
                ...
                ax.annotate(xy=(rwin_arr[:, 0].min(), ymin), 
                            text=tag.replace("_", " "), fontsize=fontsize,
                            zorder=14)
                ymin -= dy
    ...
    def plot_amplitude_threshold(self, ax, obs):
        ...
        ax.annotate(
                text=f"{self.config.win_amp_ratio * 100:.0f}% peak amp. obs.", 
