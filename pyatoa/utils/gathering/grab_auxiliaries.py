...
def _hardcode_paths():
    """
    convenience function to hardcode in a path dictionary for fetching
...
def grab_geonet_moment_tensor(event_id):
    ...
    :return:
    """
    with open(_hardcode_paths()['geonet_mt']) as f:
        reader = csv.reader(f)
...
def grab_gcmt_moment_tensor(datetime, magnitude):
    ...
    fpath = os.path.join(_hardcode_paths()['gcmt_mt'], year, fid)
    try:
...
def gcmt_and_geonet_mt(event_id):
    """
    grab both gcmt and geonet moment tensor information
    """
...
def timeshift_halfduration(gcmt_event, geonet_list):
    ...
    """
    centroid_time = gcmt_event.origins[0].time
    hypocenter_time = [i.time for i in gcmt_event.origins
