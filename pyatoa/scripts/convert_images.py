...
if __name__ == "__main__":
    image_path = "/Users/chowbr/Documents/subduction/tomo/adjoint_test/" \
                 "adjoint_master/figures/2016p858260/"
    # for images in glob.glob(image_path):
    images = image_path
    event_id = os.path.basename(images[:-1])
    #if os.path.exists(os.path.join(images,"{}_composite.pdf".format(event_id))):
    #    continue
    print(event_id)
    ds_path = "/Users/chowbr/Documents/subduction/tomo/adjoint_test/" \
              "adjoint_master/hdf5/{}.h5".format(event_id)
    ds = pyasdf.ASDFDataSet(ds_path)
    tile_images(images)
